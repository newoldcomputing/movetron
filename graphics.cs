...
namespace Movetron
{
	...
	static public class GamePanel
	{
		...
		public const byte MSG_ENEMY_DOWN = 4;
		public const byte MSG_BOMB_PLANTED = 5;
		public const byte MSG_FOUND_AMMO = 6;
		public const byte MSG_RED_KEY = 7;
		public const byte MSG_GREEN_KEY = 8;
		public const byte MSG_BLUE_KEY = 9;
		public const byte MSG_SEE_ENEMY = 10;
		public const byte MSG_ENEMY_SEES = 11;
		public const byte MSG_FOUND_BOMB = 12;
		public const byte MSG_FOUND_LIFE = 13;
		...
		static public string[,] FeedMsgs = new string[,]
		{
			...
			{"FOUND BOMB","ПОДОБРАЛ БОМБУ"},
			{"ROBOT ADDED","ЕЩЕ РОБОТ..."}
		};
		static int CurPos = 34;		//Позиция курсора в экранчике.		
		static byte LastMSGNum;		//Последнее сообщение (чтобы не повторять)			
		...
		static public void SendMsg(byte MsgNum)
		{
			//Некоторые сообщения не повторяются
			if (LastMSGNum == MSG_COMPLETE && MsgNum == MSG_COMPLETE)
				return;
